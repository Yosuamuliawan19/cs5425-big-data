...
//            g2.vertices().sort(g2.vertices().col("distance")).show();
    ...
    public static void main(String[] args) throws Exception {
        ...
        oneway = oneway.select(oneway.col("from"), functions.lead(oneway.col("from"), 1).over(window).as("to"));
        Dataset<Row> biway_forward = biway.select(biway.col("from"), functions.lead(biway.col("from"), 1).over(window).as("to"));
        Dataset<Row> biway_backward = biway.select(biway.col("from").as("to"), functions.lead(biway.col("from"), 1).over(window).as("from"));

        Dataset<Row> all_edges = oneway.union(biway_forward).union(biway_backward).filter("from IS NOT NULL AND to IS NOT NULL");
...
//        adj_list.show();
        ...
        // write part 1
//        adj_list.coalesce(1).write().option("header", "false").mode("append").save(OSM_OUTPUT);
